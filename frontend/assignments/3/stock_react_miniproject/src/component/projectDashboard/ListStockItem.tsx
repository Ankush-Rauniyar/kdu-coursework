...
export  default function ListStockItem(prop:Readonly<ItemProp>) {
  ...
  return (
    ...
    <div>{'\u20B9'}{prop.basePrice}</div>
    <button onClick={() => saveToWatchList(prop.stockName)} className={classes.button_last}
    onMouseEnter={() => setCrossHover(true)} onMouseLeave={() => setCrossHover(false)}>
      {prop.status == "plus" ? <i className="fi fi-bs-add" style={{ color: '#1971c2' }}></i>
       :
       (crossHover ? 
