...
export default function Dashboard() {
  ...
  return (
    <>
        <div className={classes.titleDashBoard}>  <span
    style={{ cursor: 'pointer', borderBottom: selectedView === 'explore' ? '3px solid #1971c2' : 'none' }}
    onClick={() => setSelectedView('explore')}
  >
    Explore
  </span>
  <span
    style={{ cursor: 'pointer', marginLeft: '1rem', borderBottom: selectedView === 'wishlist' ? '3px solid #1971c2' : 'none' }}
    onClick={() => setSelectedView('wishlist')}
  >
    My WatchList
  </span>
        </div>
        {selectedView === "explore" ? <AllStocksDashBoard stocks={stocks}/> : <WishListDashBoard stocks={wish}/>}
