...
export default function Summarizer() {
  ...
  const summaryStocks = useSelector((state: RootState) => state.summaryOfStocks.stocksSummary);
  ...
  useEffect(() => {
    const calculateStocks = async () => {
      try {
        workerRef.current = new Worker(new URL('./StockWorker.ts', import.meta.url));
  ...
  return (
    <div>
      ...
      {results.map((result, index) => (
        <div className={classes.particular_item} style={{backgroundColor : result.maxProfit >= 0  ? "#1971c2" : "#e24545"}}key={index}>
          <div className='left_container'>
            <div className={classes.stock_name_container}>
              {result.company}
            </div>
            <div className='profit_margin_container'>
              Profit Margin : {'\u20B9'}{result.maxProfit}
            </div>
          </div>
          <div className={classes.right_conatinar}>
            <div className={classes.bought_container}>
              Buy: {'\u20B9'}{result.buyRate} on {formatDate(result.buyDate)}
            </div>
            <div className={classes.sold_container}>
              Sell: {'\u20B9'}{result.sellRate} on {formatDate(result.sellDate)}
            </div>
          </div>
        </div>
      ))}
