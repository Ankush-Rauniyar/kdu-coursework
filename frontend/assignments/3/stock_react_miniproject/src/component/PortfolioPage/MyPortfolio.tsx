...
const MyPortfolio: React.FC = () => {
  ...
  const filteredTransactions = allTransactions
    .filter((transaction) => {
      return (
        transaction.stock_name.toLowerCase().includes(textFilter.toLowerCase()) ||
        transaction.stock_symbol.toLowerCase().includes(textFilter.toLowerCase())
      );
    })
    .filter((transaction) => {
      if (startDateFilter) {
            ...
            <ul className={classes.list_transaction}>
              {transactions.map((transaction, idx) => (
                <li
                  ...
                  style={{
                    opacity: selectedStocks.length > 0 && !selectedStocks.includes(transaction.stock_name)
                      ? 0.5
                      : 1,
                  }}
                >
                  <div style={{width:"25vw"}}>{transaction.stock_name}</div>
                  ...
                  <div>
                    {<i style={{ color: transaction.status === "Passed" ? "#6bb97a" : "#e76d6d" }} className="fi fi-ss-circle-small"></i>}
                  </div>
                </li>
              ))}
            </ul>
