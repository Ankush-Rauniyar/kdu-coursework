...
export function StockMain() {
    ...
    const currentStock = allStocks.find((stock) => stock.stock_name === stockName);
  ...
  const [historyTransactions, setHistoryTransactions] = useState<ParticularTransaction[]>([]);
  const [stockShares, setStockShares] = useState<Map<string, number>>(new Map());
  ...
  const handleBuy = () => {
      ...
      if (total <= currentBalance) {
          ...
          setStockShares(new Map(stockShares.set(currentStock?.stock_name ?? '', currentShares + qty)));

          setSnackbarMessage(`Success: Bought ${qty} shares at ${'\u20B9'}${prices[prices.length - 1] } each.`);
  ...
  const handleSell = () => {
      ...
      if (qty <= currentShares) {
          ...
          setCurrentBalance(currentBalance + qty * (prices[prices.length - 1]  ?? 0));
          setHistoryTransactions([...historyTransactions, tempTransaction]);
          dispatch(addTransaction(transaction));
          setStockShares(new Map(stockShares.set(currentStock?.stock_name ?? '', currentShares - qty)));

          setSnackbarMessage(`Success: Sold ${qty} shares at ${'\u20B9'}${prices[prices.length - 1] } each.`);
            setSnackbarStatus('success');
            setSnackbarOpen(true);
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      if (stockChangeContainerRef.current) {
        stockChangeContainerRef.current.scrollLeft = stockChangeContainerRef.current.scrollWidth;
      }
    }, 1000);
      ...
      <div className={classes.whole_container}>
          <div className={classes.left_stock_container}>
              <div className={classes.top_action_container}>
                  ...
                  <div className={classes.price_percentage_container}>
                      Price<span className={classes.currentPrice} style={{color: curPer > 0 ? "#34a148" : "#e54b4b"}}>{prices[prices.length - 1] }</span>
                      <span className="arrowOfChange">{curPer > 0 ? <i style ={{color : "#34a148"}}className="fi fi-br-arrow-up"></i> : <i style ={{color : "#e54b4b"}}className="fi fi-br-arrow-down"></i>}</span>
                      <span className={classes.percentageChange}>{curPer.toFixed(2)}%</span>
                  </div>
              ...
              <div ref={stockChangeContainerRef} className={classes.stock_change_container}>
                  {barData.map((amount, index) => (
                      <BarGraph key={index} amount={amount} currentPrice={prices[index-1]  ?? 0} />
          ...
          <div className={classes.history_container}>
                  ...
                  <ul>
                      {historyTransactions
                          .map((transaction) => (
                            ...
                            <div className={classes.list_right_side} style={{color : transaction.action == "BUY" ?"#34a148" :"#e54b4b"}}>
                              {transaction.action}
                            </div>
                            </li>
                          ))}
                ...
                <ul style={{paddingLeft:"0"}}>
                    {news.map((demo, index) => (
                    <li className={classes.news_item}key={index}>{demo}</li>
                    ))}
                </ul>
                </div>
