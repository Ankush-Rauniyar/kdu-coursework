    ...
    reducers:{
      updatePrice:(state,action : PayloadAction<string>)=>{
        ...
        addToWishList: (state, action: PayloadAction<string>) => {
            const current = state.stocksArray.find((stock) => stock.stock_name === action.payload) as IStockAPI;
            if (current.stock_status === 'plus') {
              current.stock_status = 'saved';
              state.wishlistStocks.push(current);
            } else {
              const indexToRemove = state.wishlistStocks.findIndex((stock) => stock.stock_name === action.payload);
              if (indexToRemove !== -1) {
          ...
          addTransaction:(state,action:PayloadAction<IStockTransaction>)=>{
            ...
            console.log(state.personalTransactions);
          }    
    },
...
export const{ addToWishList,addTransaction,updatePrice} = StockState.actions;
